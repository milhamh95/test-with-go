...
func init() {
	flag.Int64Var(&timeoutFlag, "timeoutMultiplier", 1, "set this to a higher value if you are on a slower pc and expericening test failure due to unexpected timeouts")
}
...
func TestDoThing_inject(t *testing.T) {
	...
	timeAfter = func(time.Duration) <-chan time.Time {
		ch := make(chan time.Time)
		return ch
		// it will return automcatically, always pass
		// timeout that never actually happen
